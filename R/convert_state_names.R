...
#' @param to string, format after convertion, take values of "full", "lowercase",
#' and "abbr".
#'
...
convert_state_names <- function(states, to) {
    ...
    full <- c("Alabama", "Alaska", "Arizona", "Arkansas", "California", "Colorado",
              ...
              "Northern Mariana Island", "Puerto Rico", "United States Virgin Islands",
              "United States")

    lowercase <- c("alabama", "alaska", "arizona", "arkansas", "california", "colorado",
                   "connecticut", "delaware", "district of columbia", "florida",
    ...
    # conversion
    to_be_converted <- data.table(from = states)
    converter <- data.table(from = get(from),
                            to = get(to))
